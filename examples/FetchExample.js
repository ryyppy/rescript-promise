// Generated by ReScript, PLEASE EDIT WITH CARE
'use strict';

var $$Promise = require("../src/Promise.js");
var Belt_Array = require("bs-platform/lib/js/belt_Array.js");
var NodeFetch = require("node-fetch");
var Caml_exceptions = require("bs-platform/lib/js/caml_exceptions.js");

globalThis.fetch = NodeFetch;

var NodeFetchPolyfill = {};

var $$Response = {};

function login(email, password) {
  var body = {
    email: email,
    password: password
  };
  var params = {
    method: "POST",
    headers: {
      "Content-Type": "application/json"
    },
    body: JSON.stringify(body)
  };
  return $$Promise.$$catch($$Promise.map($$Promise.$$then(globalThis.fetch("https://reqres.in/api/login", params), (function (res) {
                        return res.json();
                      })), (function (data) {
                    var msg = data.error;
                    if (!(msg == null)) {
                      return {
                              TAG: /* Error */1,
                              _0: msg
                            };
                    }
                    var token = data.token;
                    if (token == null) {
                      return {
                              TAG: /* Error */1,
                              _0: "Didn't return a token"
                            };
                    } else {
                      return {
                              TAG: /* Ok */0,
                              _0: token
                            };
                    }
                  })), (function (e) {
                var msg;
                if (e.RE_EXN_ID === $$Promise.JsError) {
                  var msg$1 = e._1.message;
                  msg = msg$1 !== undefined ? msg$1 : "";
                } else {
                  msg = "Unexpected error occurred";
                }
                return {
                        TAG: /* Error */1,
                        _0: msg
                      };
              }));
}

var Login = {
  login: login
};

function getProducts(token, param) {
  var params = {
    Authorization: "Bearer " + token
  };
  return $$Promise.$$catch($$Promise.map($$Promise.$$then(globalThis.fetch("https://reqres.in/api/products", params), (function (res) {
                        return res.json();
                      })), (function (data) {
                    var data$1 = data.data;
                    var ret = (data$1 == null) ? [] : data$1;
                    return {
                            TAG: /* Ok */0,
                            _0: ret
                          };
                  })), (function (e) {
                var msg;
                if (e.RE_EXN_ID === $$Promise.JsError) {
                  var msg$1 = e._1.message;
                  msg = msg$1 !== undefined ? msg$1 : "";
                } else {
                  msg = "Unexpected error occurred";
                }
                return {
                        TAG: /* Error */1,
                        _0: msg
                      };
              }));
}

var Product = {
  getProducts: getProducts
};

var FailedRequest = Caml_exceptions.create("FetchExample.FailedRequest");

$$Promise.$$catch($$Promise.map($$Promise.$$then(login("emma.wong@reqres.in", "pw"), (function (ret) {
                if (ret.TAG !== /* Ok */0) {
                  return Promise.reject({
                              RE_EXN_ID: FailedRequest,
                              _1: "Login error - " + ret._0
                            });
                }
                console.log("Login successful! Querying data...");
                return getProducts(ret._0, undefined);
              })), (function (result) {
            if (result.TAG === /* Ok */0) {
              console.log("\nAvailable Products:\n---");
              return Belt_Array.forEach(result._0, (function (p) {
                            console.log(String(p.id) + " - " + p.name);
                            
                          }));
            }
            console.log("Could not query products: " + result._0);
            
          })), (function (e) {
        if (e.RE_EXN_ID === FailedRequest) {
          console.log("Operation failed! " + e._1);
        } else {
          console.log("Unknown error");
        }
        
      }));

exports.NodeFetchPolyfill = NodeFetchPolyfill;
exports.$$Response = $$Response;
exports.Login = Login;
exports.Product = Product;
exports.FailedRequest = FailedRequest;
/*  Not a pure module */
