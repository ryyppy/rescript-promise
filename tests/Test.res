...
@bs.scope("Error") @bs.val external captureStackTrace: {..} => unit = "captureStackTrace"
@bs.module("@babel/code-frame") @bs.val external codeFrameColumns: string => {..} => {..} => string = "codeFrameColumns"
@bs.module("fs") @bs.val external readFileSync: string => {..} => string = "readFileSync"
